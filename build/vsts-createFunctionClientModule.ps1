...
$parentModule = 'AzDnsAsCode'
if (-not $ModuleName) { $ModuleName = 'AzDnsAsCode.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\AzDnsAsCode'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\AzDnsAsCode.psd1"
$prereqHash = @{
